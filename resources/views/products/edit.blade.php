...
<head>
    ...
    <title>Edit Produk - MySembako</title>
...
<body class="bg-gray-50 min-h-screen">
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        >
                        <button type="button" onclick="tambahKategoriBaru()" class="px-3 py-2 bg-orange-500 text-white rounded-full font-bold text-xl flex-shrink-0 hover:bg-orange-600 transition-colors" title="Tambah kategori baru">
                            +
                        </button>
                    </div>
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="stok" 
                        name="stok" 
                        value="{{ old('stok', $product->stock ?? 0) }}" 
                        min="0" 
                        step="1" 
                        required
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-full focus:outline-none focus:border-orange-500 transition-colors"
                ...
                <div>
                    ...
                    >{{ old('deskripsi', $product->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($product->image)
                        <div class="mb-4 flex items-start gap-4">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-40 h-40 object-cover rounded-lg border-2 border-gray-200">
                            <div class="flex-1">
                                <p class="text-sm font-semibold text-gray-700 mb-1">Foto saat ini</p>
                                <p class="text-sm text-gray-500">Upload foto baru untuk mengganti</p>
                            </div>
                        </div>
                    @endif
    ...
    <script>
        ...
        // Fungsi dummy untuk tombol tambah kategori
